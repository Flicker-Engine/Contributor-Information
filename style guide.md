...
-   **Coroutines**: Use coroutines for asynchronous tasks like file I/O, network requests, or complex, time-sliced AI behaviors to avoid callback-heavy or complex state machine implementations.
...
-   **`flat_map` / `flat_set`**: For performance-critical code requiring associative containers, prefer `std::flat_map` and `std::flat_set` over their node-based counterparts to improve cache locality and iteration speed.
